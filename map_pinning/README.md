...
Pinning is not required to access maps from within the same program (deamon?) however, pinning is required if other userspace applications need to interact with those maps
...
* Pin the map using map id 
    - Note: "test_map" name differs from the kernel program's map declaration. 
